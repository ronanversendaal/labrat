...
async fn save_settings(pool: &sqlx::SqlitePool, settings: &Settings) -> TauriResult<()> {
    sqlx::query(
        "INSERT OR REPLACE INTO settings (id, theme, mr_refresh_interval_seconds, cache_size_mb, ai_auto_analyze,
         ...
         font_family_ui, font_family_code)
         VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    )
    ...
    .bind(&settings.font_family_ui)
    .bind(&settings.font_family_code)
    .execute(pool)
    .await
...
fn parse_settings_row(
    (theme, refresh_interval, cache_size, ai_auto, ai_provider_id, sidebar, diff_mode, whitespace, font_size, shortcuts, font_ui, font_code): (String, i32, i32, i64, Option<String>, i64, String, i64, i32, i64, Option<String>, Option<String>),
) -> Settings {
    Settings {
        theme: Theme::from_db_str(&theme),
        ...
        font_family_ui: font_ui,
        font_family_code: font_code,
    }
}
...
pub async fn get_inner(state: &SharedAppState) -> TauriResult<Settings> {
    ...
    let row = sqlx::query_as::<_, (String, i32, i32, i64, Option<String>, i64, String, i64, i32, i64, Option<String>, Option<String>)>(
        SELECT_SETTINGS
    )
